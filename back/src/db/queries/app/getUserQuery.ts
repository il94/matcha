export const getUserQuery = `
	SELECT
		...
		COUNT(DISTINCT user_votes.user_id) FILTER (WHERE user_votes.liked = TRUE) AS likes,
		COUNT(DISTINCT user_votes.user_id) AS views,
		(
			SELECT COUNT(*)
			FROM user_votes uv1
			JOIN user_votes uv2
	...
	LEFT JOIN user_votes ON user_votes.target_id = users.id
	WHERE users.id = $1
	GROUP BY users.id;
`
